  ...
  for (const item in query) {
    if (item) {
      ...
      if (value === null || value === undefined || value === "") {
        ...
        columns?.forEach((e) => {
          //https://stackoverflow.com/questions/57798016/how-to-ignore-type-instantiation-is-excessively-deep-and-possibly-infinite-ts
          //@ts-ignore
 ...
 * 5. form表单（搜索和编辑中）中select支持异步远程获取结果：支持EasyAsyncSelectProps属性转换成request函数
...
export const EasyProTable = <T extends BaseRecord, Q extends BasePageQuery = BasePageQuery>(props: EasyProTableProps<T, Q> & Omit<ProTableProps<T, Q, 'text'>, 'params' | 'request' | 'dataSource'>) => {
  ...
  useBus('cacheUpdate-' + props.listApi, () => {
    if (EasyProConfig.EnableLog) console.log("recv cacheUpdate notify, refresh=" + refreshCount)
  ...
  useBus((e: EventAction) => e.type.indexOf("refreshList") >= 0,
    (e: EventAction) => {
      ...
      if (EasyProConfig.EnableLog) console.log("recv notify type=" + type)
  ...
  const searchReset = () => {
    ...
    current.query = q
    if (EasyProConfig.EnableLog) console.log("after searchReset: " + JSON.stringify(q))
    setUseCache(false)
    setQuery(q)
  }
  ...
  const search = (params: Q) => {
    if (EasyProConfig.EnableLog) console.log("search: " + JSON.stringify(params))
    resetPagination()
  ...
  //if (EasyProConfig.EnableLog) console.log("isLoading=" + isLoading + ", current.query=" + JSON.stringify(current.query)+ ", list=" + JSON.stringify(list))
  ...
  return <RouteContext.Consumer>
    {(value: RouteContextType) => {
      ...
      const EasyToolBarRender = props.toolBarRender ? props.toolBarRender :( (props.disableAdd || !props.saveApi) ? undefined: () => [
     ...
     // const columns = useMemo(() => { return applyinitalQuery(actions, props.saveApi, props.delApi, props.columns, props.initialQuery) }, [])
      ...
      return <div>
        <ProTable<T, Q>
          ...
          onReset={searchReset}
          onSubmit={search}
          toolBarRender={EasyToolBarRender}
  ...
  if (typeof editFormConfig === "function") {
    ...
    if (path) {
      if (path === 'ModalForm' || path === 'DrawerForm') {
        ...
        if (style === 'Button') {
          //if(EasyProConfig.EnableLog)console.log("prepare jump goto newOne")
