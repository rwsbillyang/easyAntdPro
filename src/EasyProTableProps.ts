...
export interface EasyProTableProps<T extends BaseRecord, Q extends BasePageQuery> {
  ...
  layoutType?: 'ModalForm' | 'DrawerForm'//editor中的layout类型，用于传递给EasySchemaFormEditor
 ...
 */
export interface EasyAsyncSelectProps<T extends BaseRecord, Q extends BasePageQuery = BasePageQuery> {
  key?: string //缓存键
  url: string //请求url
  query?: Q //请求参数
...
export const asyncSelectProps2Request = <T extends BaseRecord, Q extends BasePageQuery>(props: EasyAsyncSelectProps<T, Q>, cb?: (data: any) => void, method: "GET" | "POST" | "PUT" | "DELETE" = "GET") => {
...
export const asyncSelect2Request = <T extends BaseRecord>(url: string, key?: string, query?: any, method: "GET" | "POST" | "PUT" | "DELETE" = "GET", convertFunc?: (item: T) => { label: string, value?: string | number | object } ) => {
  //console.log(JSON.stringify(params))
  //const queryStr = query2Params()
  return cachedFetchPromise(url, method, query, key, StorageType.OnlySessionStorage, undefined, (bizData: any) => bizData.map((e: any) => convertFunc ? convertFunc(e) : e))
}